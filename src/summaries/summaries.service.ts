...
- item 2`;
            ...
            const extractionResponse = await this.openai.chat.completions.create({
                model: 'llama-3.1-8b-instant',
                messages: [
                    { role: "system", content: "You are an expert analyst. Extract structured information from the text." },
    ...
    private parseExtractionResponse(response: string): {
        ...
        try {
            ...
            sections.forEach(section => {
                ...
                if (trimmedSection.startsWith('ACTION_ITEMS:')) category = 'actionItems';
                else if (trimmedSection.startsWith('RISKS:')) category = 'risks';
                else if (trimmedSection.startsWith('NEXT_STEPS:')) category = 'nextSteps';
            ...
            if (result.actionItems.length === 0) result.actionItems.push('No specific action items identified');
            if (result.risks.length === 0) result.risks.push('No risks identified');
            if (result.nextSteps.length === 0) result.nextSteps.push('No next steps identified');
